...
module.exports.createPostCtrl = asyncHandler(async (req, res) => {
  ...
  if (!req.file) {
    return res.status(400).json({ message: "Nie wybrano obrazu" });
  }
...
module.exports.getSinglePostCtrl = asyncHandler(async (req, res) => {
  const post = await Post.findById(req.params.id)
    ...
    .populate("comments");
  if (!post) {
    return res.status(404).json({ message: "Postu nie znaleziono" });
  }
...
module.exports.deletePostCtrl = asyncHandler(async (req, res) => {
  const post = await Post.findById(req.params.id);
  if (!post) {
    return res.status(404).json({ message: "Posta nie znaleziono" });
  }
  if (req.user.isAdmin || req.user.id === post.user.toString()) {
    ...
    res.status(403).json({ message: "Odmowa dostępu, dostęp zabroniony" });
  }
});
